...
class evotx_helper{
	...
	function remaining_stock_html($stock, $text='', $visible=true){
		...
		// text string
		if(empty($text)){
			$text = $remaining_count>1? __('Tickes Remaining!','evotx') : __('Ticket Remaining!','evotx');
		} 
	...
	function ticket_qty_html($max='', $unqiue_class=''){
		...
		<p class="evotx_quantity">
			<span class='label'><?php evo_lang_e('How many tickets?');?></span>
	...
	function add_to_cart_html($type='good', $msg=''){
		...
		if( $type =='good'):
			...
			<span>
				<a class='evcal_btn' href="<?php echo wc_get_cart_url();?>" <?php echo $newWind;?>><?php evo_lang_e('View Cart');?></a> <em>|</em>
