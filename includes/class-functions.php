	...
	// TICKET related
		...
		// ADD NEW matching woocommerce product
			function add_new_woocommerce_product($post_id){
				...
				$title = $this->get_ticekt_product_title();
				if(!$title) return false;

				//echo $user_ID.' '.$post_id.'ttt';
			...
			function save_product_meta_values($woo_post_id, $post_id, $method='post'){
				...
				foreach($update_metas as $umeta=>$umetav){
					if($umeta == '_regular_price' || $umeta == '_sale_price'|| $umeta == '_price'){
						...
						$_stock_status = (!empty($values[$umetav]) && $values[$umetav]=='yes')? 'outofstock': 'instock';
...
// SUPPORTIVE
	    ...
	    // get product title
	    function get_ticekt_product_title(){
	    	...
	    	$event_title = (!empty($_REQUEST['post_title'])? 
				$_REQUEST['post_title']: ( !empty($_POST['event_name'])? $_POST['event_name']:''));
	    	$structure = str_replace('{event_name}', $event_title, $structure);
