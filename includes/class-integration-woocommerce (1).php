...
class EVOTX_WC{
	public function __construct(){
		...
		// checkout
			...
			add_action('woocommerce_checkout_order_processed', array($this, 'adjust_ticket_var_stock'), 10, 1);
	...
	// CART item View
		...
		// CART ticket item name
		    function cart_item_name_box($product_name, $values, $cart_item_key ) {
	            ...
	            $return_string .= "<span class='item_meta_data'>";
				...
				foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( $cart_item['product_id'] > 0 ) {
						...
						if(!evo_check_yn($event_meta, 'evotx_tix')){
							...
							wc_add_notice( 'TIcket no longer for sale!' );
						}else{
	...
	// CHECKOUT
		...
		// RESTOCK & REDUCE STOCK
			...
			function restock_stock($order){
				$order_id = $order->get_id();
				$this->adjust_ticket_var_stock($order_id,'restock');
			}
			function restock_from_orderid($order_id){
				$this->adjust_ticket_var_stock($order_id,'restock');
			}

		// Adjust ticket stock
			function adjust_ticket_var_stock($order_id, $type='reduce'){
				...
				if( empty($evo_stock_reduced) || 
					(!empty($evo_stock_reduced) && ( ($evo_stock_reduced == 'yes'&& $type =='restock') ||	($evo_stock_reduced == 'no'&& $type=='reduce') ) )
				){					
					$proceed = true;
				}
				...
				// each order item in the order
			    	foreach ( $order->get_items() as $item_id=>$item) {
			    		...
			    		if ( $item['product_id'] > 0 ) {
				    		...
				    		// if repeat interval based stock active
				    		if( $TIX_EVENT->is_ri_count_active()){
			    				...
			    				if ( isset( $item['variation_id'] ) && $item['variation_id'] ) {
									$order->add_order_note( sprintf( __( 'Item %1$s variation #%2$s stock %s from %3$s.', 'evotx' ), $item_name, $item['variation_id'], $type,$qty) );
								} else {
									$order->add_order_note( sprintf( __( 'Item %1$s stock %s from %2$s.', 'evotx' ), $item_name, $type,$qty) );
								}
				    		...
				    		// none repeating capacity activated events
				    			...
				    			// adjust event ticket stock
				    			$new_quantity = wc_update_product_stock($_product, $qty, ($type == 'reduce')?'decrease':'increase' );	
								
								if($type == 'reduce'){
									$order->add_order_note( sprintf( __( 'Item #%s stock reduced from %s to %s.', 'woocommerce' ), $item['product_id'], $old_stock, $new_quantity) );
								}else{
				    		...
				    		// pluggable
				    		$stock_reduced = apply_filters('evotx_adjust_orderitem_ticket_stockother', $stock_reduced, $TIX_EVENT, $order, $item_id, $item, $type);			    		
				    	}
			    	}
		        ...
		        foreach($items as $item => $values) { 
		        	...
		        	if($values['quantity']>0){
		        		for($x=0; $x<$values['quantity']; $x++){
		        			...
		        			$output .= "<span class='evotx_tai_oneholder_title'>". evo_lang('Ticket Holder') ." #".($x+1)."</span>";
		        			...
		        			foreach( $this->_supportive_checkout_additional_fiels($event_id, $item, $values)  as $key=>$data){
			        			...
			        			$result = woocommerce_form_field('tixholders['.$event_id.']['.$RI.']['.$x.']['.$key.']', array(
									'type' => 		$data['type'],
									...
									'return'=>		true
								) , $checkout->get_value('tixholders['.$event_id.']['.$RI.']['.$x.']['.$key.']'));
			...
			if(!empty($_POST['tixholders'])){
				...
				if($required){
					...
					foreach($_POST['tixholders'] as $event=>$RIS){
						...
						foreach($RIS as $ri=>$keys){	
							foreach($keys as $key=>$vals){
								// empty name
								if( empty($vals['name']) ) $empty = true;
								// check for minimal name length
								if( strlen($vals['name']) <2) $empty = true;
							}
						}
					}
			...
			?>
				...
				<table class="shop_table ticketholder_details" cellspacing="0">
					...
					foreach($ticket_holders as $e=>$dd){
						?><tr><th><?php _e( 'Attendee', 'evotx' ); ?></th><td><?php
						foreach($dd as $tn=>$nm){ 
	...
	// EMAILING
		...
		function order_item_name($item_name, $item){
			...
			$item_name = get_the_title($event_id);
		...
		function order_details($order, $sent_to_admin = false, $plain_text = false, $email = ''){
			...
			<h2><?php _e('Ticket Holder Details','evotx');?></h2>
